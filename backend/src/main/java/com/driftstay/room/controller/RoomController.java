...
public class RoomController {
    ...
    public ResponseEntity<ApiResponse<RoomDetailResponse>> createRoom(
            ...
            @Validated(CreateValidation.class) @RequestBody RoomCreateRequest request) {
    ...
    public ResponseEntity<ApiResponse<List<RoomSummaryResponse>>> getRooms(@PathVariable Long propertyId) {
        var rooms = roomMapper.toSummaryList(roomService.getRoomsByPropertyId(propertyId));
    ...
    public ResponseEntity<Void> deleteRoom(@PathVariable Long propertyId, @PathVariable String publicId) {
        roomService.deleteRoom(publicId);
        return ResponseEntity.noContent().build();
    }

    private RoomDetailResponse enrichDetail(RoomDetailResponse detail, Room room) {
        var imageUrls = room.getImages().stream().map(img -> img.getImageUrl()).toList();
        return RoomDetailResponse.builder()
                .id(detail.getId()).publicId(detail.getPublicId())
                .roomName(detail.getRoomName()).roomNumber(detail.getRoomNumber())
                .description(detail.getDescription()).roomType(detail.getRoomType())
                .capacity(detail.getCapacity()).bedCount(detail.getBedCount())
                .bedType(detail.getBedType()).bathroomCount(detail.getBathroomCount())
                .basePrice(detail.getBasePrice()).weekendPrice(detail.getWeekendPrice())
                .cleaningFee(detail.getCleaningFee()).extraGuestFee(detail.getExtraGuestFee())
                .areaSqft(detail.getAreaSqft()).floorNumber(detail.getFloorNumber())
                .status(detail.getStatus()).imageUrls(imageUrls)
                .build();
    }
}
